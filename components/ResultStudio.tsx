...
interface ResultStudioProps {
  initialImages: { NORMAL: string; CLICKBAIT: string };
  summary: string;
  onEdit: (image: string, prompt: string) => Promise<string>;
  onReset: () => void;
}
...
export const ResultStudio: React.FC<ResultStudioProps> = ({ initialImages, summary, onEdit, onReset }) => {
  ...
  // Store current images for both modes
  const [images, setImages] = useState(initialImages);
  
  // Store history for both modes
  const [history, setHistory] = useState<{ NORMAL: string[]; CLICKBAIT: string[] }>({
  ...
  const currentImage = images[activeMode];
  const currentHistory = history[activeMode];

  const handleDownload = () => {
    downloadImage(currentImage, `thumbnail-pro-${activeMode.toLowerCase()}-${Date.now()}.png`);
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newImage = await onEdit(currentImage, editPrompt);
      ...
      setHistory(prev => ({
        ...prev,
        [activeMode]: [...prev[activeMode], newImage]
      }));
  ...
  const handleUndo = () => {
    if (currentHistory.length > 1) {
      ...
      setHistory(prev => ({
        ...prev,
        [activeMode]: newHistoryList
      }));
      
      setImages(prev => ({
        ...prev,
        [activeMode]: previousImage
      }));
    }
  };
    ...
    <div className="w-full max-w-6xl flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="relative group rounded-xl overflow-hidden shadow-2xl border border-zinc-800 bg-black aspect-video">
             <img 
               src={currentImage} 
             ...
             {isEditing && (
               <div className="absolute inset-0 bg-black/60 flex items-center justify-center backdrop-blur-sm">
                  <div className="flex flex-col items-center">
                      ...
                      <p className="text-xl font-bold text-white">Applying Edits to {activeMode} thumbnail...</p>
                  </div>
               </div>
             )}
          </div>
        ...
        <div className="w-full lg:w-96 flex flex-col gap-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl">
              ...
              <p className="text-sm text-gray-400 mb-4">
                  Editing the <strong>{activeMode}</strong> thumbnail. Ask for changes like "Make the text bigger" or "Add fire effects".
              </p>
